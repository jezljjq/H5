...
class FlowRunner:
    ...
    def run_window(self, hwnd: int) -> RunResult:
        try:
            ...
            while index < len(flow):
                ...
                result = self._run_click_step(hwnd, step)
                if not result.ok:
                    return result
            ...
            message = f"窗口 {hwnd} 执行异常: {exc}"
            self.log(message)
            return RunResult(False, message)

    def _run_click_step(self, hwnd: int, step: FlowStep) -> RunResult:
        ...
        self.log(f"窗口 {hwnd}: 开始识别 {step.name}")
        ...
        for attempt in range(1, params.retries + 1):
            ...
            if recognition.success:
                ...
                if params.found_action in {"click", "click_jump", ""}:
                    ...
                    self.log(f"窗口 {hwnd}: {step.name} 命中 {recognition.template_name} {recognition.score:.3f}，点击 ({recognition.x}, {recognition.y}){target_text}{method_text}，后端 {recognition.backend}")
                    self.sleep(params.click_wait)
                elif params.found_action == "jump":
                    self.log(f"窗口 {hwnd}: {step.name} 命中 {recognition.template_name} {recognition.score:.3f}，跳转，后端 {recognition.backend}")
                else:
            ...
            self.log(f"窗口 {hwnd}: {step.name} 第 {attempt}/{params.retries} 次未命中，后端 {recognition.backend}，原因 {recognition.error}")
